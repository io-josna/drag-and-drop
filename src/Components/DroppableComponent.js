...
const DroppableComponent = ({ data }) => {
  ...
  const maxColumns = 6;
  const availableColumnsInRow =
    maxColumns - (droppedItemIds.length % maxColumns);
  const gridColumnCount = `repeat(${availableColumnsInRow}, 1fr)`;
  const gridStyles = {
    display: "grid",
    gridTemplateColumns: gridColumnCount,
    gap: "8px",
